...
///
/// `Config.swift` is auto-generated and its values are only injected at build
/// time — in a standalone Xcode checkout those strings are empty, which makes
/// the auth base URL blank and produces an "unsupported URL" error at sign-in.
/// These values are public client keys (safe to embed), so we provide them here
/// as a fallback whenever `Config` resolves to an empty string.
enum AuthConfig {
    /// Hosted auth API base URL.
