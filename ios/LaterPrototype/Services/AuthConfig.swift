...
/// Later's build pipeline — in a standalone Xcode checkout those strings are
...
enum AuthConfig {
    /// Later's hosted auth API base URL.
    private static let fallbackAuthURL = "https://api.later.com"
    ...
    static var authURL: String { resolve(Config.EXPO_PUBLIC_LATER_AUTH_URL, fallbackAuthURL) }
    static var appKey: String { resolve(Config.EXPO_PUBLIC_LATER_APP_KEY, fallbackAppKey) }
