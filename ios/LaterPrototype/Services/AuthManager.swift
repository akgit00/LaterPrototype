...
class AuthManager {
    ...
    private var developerHint: String? {
        UserDefaults.standard.string(forKey: "LATER_DEVELOPER_HINT")
    }
    ...
    private func getRefreshToken() -> String? {
        #if targetEnvironment(simulator)
        if let ud = UserDefaults.standard.string(forKey: "LATER_AUTH_REFRESH_TOKEN") {
            return ud
        }
        #endif
    ...
    private func runWebAuthSession(authURL authURLString: String) async throws -> String {
        let callbackScheme = "later-\(projectID)"
    ...
    func signOut() async {
        ...
        UserDefaults.standard.removeObject(forKey: "LATER_AUTH_REFRESH_TOKEN")
        user = nil
    }
