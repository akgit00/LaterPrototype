...
///
/// `Config.swift` only has real values when the app is built inside Later's
/// pipeline — in a standalone Xcode checkout those strings are empty. The
...
enum SupabaseConfig {
    ...
    // Later-injected values, which point at the auto-provisioned project.
    ...
    static var callbackScheme: String { "later-\(projectID)" }
